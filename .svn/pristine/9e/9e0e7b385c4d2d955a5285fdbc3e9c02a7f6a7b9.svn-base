...
public class OracleBatchInsertPlugin extends PluginAdapter {
    ...
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass,
        ...
        if (introspectedTable.getTargetRuntime() == IntrospectedTable.TargetRuntime.MYBATIS3) {
            MethodGeneratorTool
                    .defaultBatchInsertOrUpdateMethodGen(MethodGeneratorTool.INSERT, interfaze, introspectedTable,
                            context);
        }
    ...
    private void addSqlMapper(Document document, IntrospectedTable introspectedTable) {
        //1.Batchinsert
        XmlElement baseElement = SqlMapperGeneratorTool
                .baseElementGenerator(SqlMapperGeneratorTool.INSERT, BATCH_INSERT,
                        FullyQualifiedJavaType.getNewListInstance());

        XmlElement foreachElement = SqlMapperGeneratorTool
                .baseForeachElementGenerator(PARAMETER_NAME, "item", "index", "union all");

        //tableName
        baseElement.addElement(new TextElement(
                String.format("insert into %s (", introspectedTable.getFullyQualifiedTableNameAtRuntime())));
