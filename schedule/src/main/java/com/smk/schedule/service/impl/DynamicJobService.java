...
public class DynamicJobService {
    ...
    public void addJob(Job job) {
        ...
        JobCoreConfiguration coreConfig = JobCoreConfiguration
                ...
                .jobProperties(JobPropertiesEnum.EXECUTOR_SERVICE_HANDLER.getKey(),
                        job.getJobProperties().getExecutorServiceHandler()).build();
        ...
        //jobType为空时，默认simplejob
        if (StringUtils.isEmpty(jobType) || jobType.equals("SIMPLE")) {
        ...
        defaultListableBeanFactory
                .registerBeanDefinition("SpringJobScheduler" + job.getJobName(), factory.getBeanDefinition());
        SpringJobScheduler springJobScheduler = (SpringJobScheduler) ctx
                .getBean("SpringJobScheduler" + job.getJobName());
    ...
    public void monitorJobRegister() {
        CuratorFramework client = zookeeperRegistryCenter.getClient();
        @SuppressWarnings("resource") PathChildrenCache childrenCache = new PathChildrenCache(client, "/", true);
        PathChildrenCacheListener childrenCacheListener = new PathChildrenCacheListener() {
            public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
                ChildData data = event.getData();
                switch (event.getType()) {
                case CHILD_ADDED:
                    String config = new String(client.getData().forPath(data.getPath() + "/config"), CommonConstant.DEFAULT_ENCODING);

                    Job job = JsonUtil.json2Object(config, Job.class);
                    Object bean = null;
                    // 获取bean失败则添加任务
                    try {
